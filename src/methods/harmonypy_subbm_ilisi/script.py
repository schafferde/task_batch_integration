...
def column_ilisi(i):
    adata_tmp = ad.AnnData(X=out[:, i].reshape((-1,1)), obs={"batch":adata.obs['batch'].values})
...
category_means = pd.DataFrame(out).groupby(adata.obs['batch'].values).transform('mean').values
print("Means:", category_means.shape)
result = out - (scores * category_means)
