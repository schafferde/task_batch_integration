...
- `src/control_methods/` contains a panel of seven control methods used to calculate empirical minimum and maximum ranges for each metric and dataset. We corrected the behavior of the `embed_cell_types` control method to remove random noise, and differentiate the behavior from `embed_cell_types_jittered` (unchanged).  
...
- `scripts/` contains some added scripts that we used for running the pipeline and associated data handling. Scripts not mentioned are included from
    ...
    - Added: `scripts/scale_metrics.py`, which takes two score CSVs, one for batch-correction methods and one for a panel of control methods, and produces a scaled score matrix for the batch-correction methods. The scaling follows the behavior of the OpenProblems website, in which a maximimum and minimum value is found for each (metric, dataset) pair. Then, the metric values for the "real" methods are mapped to the corresponding range, taking the control minimum and 0 and the control maximum as 1. Observed metric values that fall outside of the control range are floored/capped at 0/1. This approach generates a scaling that is independent of the set of "real" methods benchmarked. 
